...
document.addEventListener('turbo:load', () => {
  if (!document.querySelector('[data-player-id]') || !document.querySelector('[data-game-id]')) {
    return;
  }
  ...
  consumer.subscriptions.create(
    ...
    },
    {
      connected () {
        this.player_id = document.querySelector('[data-player-id]').dataset.playerId;
        this.game_id = document.querySelector('[data-game-id]').dataset.gameId;
        console.log('Connected to GamePlayerChannel');
      },
...
const convertToRoman = (count) => {
  ...
  const selectedButtons = getRandomButtons(count);
  selectedButtons.forEach((button) => {
    if (Number(button.dataset.number) === 23) {
      return;
    }
